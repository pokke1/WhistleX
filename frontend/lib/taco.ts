...
function resolveTacoConfig({
  ...
  const conditionChain =
    conditionChainId || Number(process.env.NEXT_PUBLIC_TACO_CONDITION_CHAIN_ID) || DEFAULT_CONDITION_CHAIN_ID;
...
export async function encryptWithTaco({
  ...
  const amoyProvider = new providers.JsonRpcProvider(condition || DEFAULT_POLYGON_AMOY_RPC_URL);
  const encryptorWallet = new Wallet(key, amoyProvider);
