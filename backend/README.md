...
`.env.example` is prefilled with the Polygon Amoy defaults from the TACo CLI sample (DRPC Amoy RPC + demo deployer/TACo key). Leave `.env` empty to reuse those, or populate it with your own RPC/key when moving to another network.
