...
async function main() {
  ...
  console.log("Network:", network.name || "amoy", "(chain id:", network.chainId, ")");
  console.log(`Deployer balance: ${balanceMatic.toFixed(6)} MATIC`);

  const MIN_REQUIRED_MATIC = 0.05; // ~50 finney safety buffer for two txs

  if (balanceMatic < MIN_REQUIRED_MATIC) {
    throw new Error(
      `Insufficient funds on ${network.name || "amoy"}. ` +
        `The deployer ${deployer.address} only has ${balanceMatic.toFixed(6)} MATIC. ` +
        `Fund this address on Polygon Amoy (e.g., via a faucet) or set DEPLOYER_KEY to a funded key before rerunning.`
    );
  }
