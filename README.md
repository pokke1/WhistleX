...
For end-to-end TACo + Amoy testing, the repo now uses the same demo values as the provided CLI sample:

- RPC / DKG RPC: `https://polygon-amoy.drpc.org`
- Ritual ID: `6`
- Demo private key: `0x8768...d23a` (stored in `shared/testnet.ts`)

Environment variables in `frontend/.env.example` and `backend/.env.example` are prefilled with these values. If you populate `.env` files, those overrides are used; if you leave them empty, the hardcoded Amoy defaults remain in effect for fast testing.
...
- `backend/.env`: set `FACTORY_ADDRESS` to the factory address printed by the deploy script (RPC defaults to Amoy unless you set `RPC_URL`).
